...
UTEST_F_SETUP(allow_unquoted_keys) {
...
UTEST_F_TEARDOWN(allow_unquoted_keys) {
...
UTEST_F(allow_unquoted_keys, read_write_pretty_read) {
  size_t size = 0;
...
UTEST_F(allow_unquoted_keys, read_write_minified_read) {
  size_t size = 0;
